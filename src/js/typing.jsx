...
class Text extends Component {
  ...
  updateOldDataToNew() {
    for (let i in this.everyWordsProb) {
      if (typeof this.everyWordsProb[i] != "number") {
        this.everyWordsProb[i] =
          this.everyWordsProb[i][0] / (this.everyWordsProb[i][1] + 1);
      }
    }
  }
  ...
  updateWordsSelect() {
    // 使可选字列表长度+1，最低为5
    const t = Math.max(this.wordsSelect.length + 1, 5);
    // 最长不能超过全部可选字库
    let len = Math.min(t, words.length);
  ...
  selectFromWords() {
    ...
    while (i < num) {
      const index = Math.min(
        wdLen - 1,
        Math.floor(Math.random() * (1 + wdLen))
      );
  ...
  text() {
    ...
    console.log(
      ...
      wordsSpeed,
      this.props.goalWordSpeed,
      this.props.currentGrade,
      this.props.goalSpeed
    );

    if (
      (this.props.currentGrade >= this.props.goalSpeed &&
        wordsSpeed >= this.props.goalWordSpeed) ||
      this.wordsSelect.length < 5
    ) {
      let allOk = true;
  ...
  OneCharacter(c) {
    return (
      <OneCharacter
        key={`index_${c.index}`}
        c={c}
        current={this.state.currentIndex === c.index}
      />
    );
  }
  ...
  calculateOk() {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      const currentProb = Math.max(
        0,
        Math.min(1.0, this.everyWordsProb[c] || 0.5)
      );
    ...
    for (let i = 0; i < this.wordsSelect.length; i++) {
      ...
      if (this.isWordOk(c)) {
        console.log(
          "OK:",
          c,
          this.wordsSelectRandom.indexOf(c),
          this.wordsSelectRandom.lastIndexOf(c)
        );
  ...
  OneProbC(c, i) {
    ...
    const probPercentShow = Math.floor((probPercent * 100 * 10) / 255) / 10;
    ...
    return (
      <Card>
        ...
        <CardBody>
          ...
          <hr />
          <div className="flex">{this.state.text.map(this.OneCharacter)}</div>
        </CardBody>
      </Card>
    );
  }
...
class Keyboard extends Component {
  ...
  render() {
    return (
      <div ref="origin_position">
        <div>
          <Card>
            <textarea
              className="form-control form-control-sm text-muted"
              rows={this.state.simple === 1 ? 1 : 4}
              onKeyDown={this.onKeyDown}
              onChange={this.onChange}
              value={this.state.value}
              ref="input"
              style={{ backgroundColor: "transparent", border: "0 solid" }}
              placeholder="在这里输入……"
            />
          </Card>
        </div>
...
class CurrentGrade extends Component {
  constructor(props) {
    ...
    /**
     * @typedef GradeContext
     * @type {object}
     * @property {number} startTime - 开始时间
     * @property {number} endTime - 结束时间
     * @property {number} keyNum - 击键次数
     * @property {number} speed - 打字速度
     */
  ...
  onKeyDown(e) {
    ...
    let keyNum = this.currentGrade.keyNum;
    let speed;
    keyNum++;
  ...
  onEnd(grade) {
    ...
    const wordsSpeed =
      (grade.length * 1000 * 60) / (current - this.currentGrade.startTime);

    const eachWordKey =
      Math.floor(((this.currentGrade.speed * 60) / wordsSpeed) * 100) / 100;
    ...
    return (
      <Card>
        ...
        <CardBody className="flex">
          击键：{(this.state.speed * 100) / 100} {" | "}
          打字：{Math.floor(this.state.wordsSpeed * 100) / 100} {" | "}
          码长：{this.state.eachWordKey} {" | "}
          错误：{Math.floor(this.state.error * 100)}%
        </CardBody>
      </Card>
    );
  }
